...
$sql="SELECT * ,m.name, s.state_name,ci.city_name FROM camp c JOIN members m On m.member_id = c.member_id JOIN states s ON s.state_id = c.state_id JOIN city ci ON ci.city_id = c.city_id WHERE status = '1'";
$res=mysqli_query($connection,$sql);
if(mysqli_num_rows($res)>0){
    $html='<head>
    <link rel="stylesheet" href="rcss.css">
  </head><h1>Ongoing Camps</h1><table cellspacing=70 cellpadding=70 align="center" >';
    ...
    $mpdf->WriteHTML($html);
    // $file=time().'.pdf';
    $mpdf->output('Ongoing Camps Report.pdf','D');
}
else{
    $html='No data found';
}
