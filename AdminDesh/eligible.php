...
<body>
    ...
    <div class="container">
        ...
        <h2>Eligible Donors Report</h2>
        <!-- Export Link -->
        <div>
        <form method="post" style="margin-left:1000px;">
            <button type="submit" class="btn btn-outline-danger" formaction="eligible1.php"><i class="fa fa-file-pdf-o"></i> PDF</button>
            <button type="submit" class="btn btn-outline-success" formaction="eligib.php"><i class="fa fa-file-excel-o"></i> Excel</button>
        </form>
        </div><br>
        <div class="row">
            <table class="table table-striped table-bordered">
                ...
                $result=$connection->query("SELECT m.name, e.*, d.donation_status,b.blood_grp FROM eligibility e JOIN donors d ON e.member_id = d.member_id JOIN members m ON e.member_id = m.member_id JOIN blood_group b ON b.blood_id = e.blood_id
                WHERE d.donation_status = '1' ");
                if($result-> num_rows > 0){
                    while($row = $result->fetch_assoc()){
                        ?>
                        <tr>
                            <td><?php echo $row['eligible_id'];   ?></td>
                            <td><?php echo $row['name'];   ?></td>
                            <td><?php echo $row['age'];   ?></td>
                            <td><?php echo $row['weight'];   ?></td>
                            <td><?php echo $row['tattoo_test'];   ?></td>
                            <td><?php echo $row['hiv_test'];   ?></td>
