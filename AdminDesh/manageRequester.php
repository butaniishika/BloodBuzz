...
<head>
    <meta charset="utf-8">
    <title>Blood Buzz-Manage Requester</title>
    ...
    <div class="container-xxl position-relative bg-white d-flex p-0">
                ...
                <div class="navbar-nav w-100">
                    ...
                    <div class="nav-item dropdown">
                        ...
                        <div class="dropdown-menu bg-transparent border-0">
                            ...
                            <a href="manageRequester.php" class="dropdown-item active">Requester</a>
        ...
        <div class="content">
            ...
            <?php 
                ...
                try
                {
                $get_donors=$connection->prepare("SELECT b.blood_grp, m.name,m.email, r.*, COUNT(*) AS num_records
                FROM members m
                JOIN requester r ON m.member_id = r.member_id
                JOIN blood_group b ON r.blood_id = b.blood_id
                GROUP BY r.member_id, r.request_id
                LIMIT $offset, $records_per_page");
                $get_donors->execute();
                // $donors=$get_donors->fetchAll(PDO::FETCH_ASSOC);
                }
                ...
                $total_records = $connection->query("SELECT COUNT(*) FROM requester")->fetchColumn();
            ...
            <div class="container-fluid pt-4 px-4">
                ...
                <tbody>
        <?php
        $groupedDonors = []; // Initialize an array to hold grouped donors
        while ($row = $get_donors->fetch(PDO::FETCH_ASSOC)) {
            $groupedDonors[$row['member_id']][] = $row; // Group donors by member_id
        }

        foreach ($groupedDonors as $memberId => $donors) {
            foreach ($donors as $key => $donor) {
                echo '<tr>';
                echo '<td>' . $donor['request_id'] . '</td>';
                echo '<td>' . $donor['name'] . '</td>';
                echo '<td>' . $donor['patient_name'] . '</td>';
                echo '<td>' . $donor['blood_grp'] . '</td>';
                echo '<td>' . $donor['created_at'] . '</td>';
                echo '<td>';
                if ($key === 0) { // Display delete button for the first record in the group
                    echo "<form action='./deleteRequest.php?mid=$memberId&rid={$donor['request_id']}&name={$donor['name']}&email={$donor['email']}' method='post'>";
                    echo '<input type="hidden" name="member_id" value="' . $memberId . '">';
                    echo '<button type="submit" name="delete_group" class="btn btn-danger"><i class="bi bi-trash3-fill"></i> Delete Requests</button>';
                    echo '</form>';
                }
                echo '</td>';
                echo '</tr>';
