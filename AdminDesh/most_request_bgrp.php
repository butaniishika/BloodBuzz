...
<body>
    ...
    <div class="container">
        ...
        <h2>Most Requested Blood Group Report</h2>
        <!-- Export Link -->
        <div>
        <form method="post" style="margin-left:1000px;">
            <button type="submit" class="btn btn-outline-danger" formaction="mostrblood.php"><i class="fa fa-file-pdf-o"></i> PDF</button>
            <button type="submit" class="btn btn-outline-success" formaction="most_req_bgrp.php"><i class="fa fa-file-excel-o"></i> Excel</button>
        </form>
        </div><br>
        <div class="row">
            <table class="table table-striped table-bordered">
                ...
                $result=$connection->query("SELECT r.request_id, r.member_id, r.patient_name, r.created_at, bg.blood_grp,m.name FROM requester r JOIN blood_group bg ON r.blood_id = bg.blood_id JOIN members m ON m.member_id=r.member_id JOIN (
                    SELECT bg.blood_grp, COUNT(*) AS request_count FROM requester r JOIN blood_group bg ON r.blood_id = bg.blood_id GROUP BY bg.blood_grp ORDER BY request_count DESC LIMIT 1
                ) AS most_requested ON bg.blood_grp = most_requested.blood_grp ");
